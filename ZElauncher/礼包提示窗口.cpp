...
void CAwardTipUI::OnTip()
{
	...
	UINT Pos = pStr.find(_T("您已获得"));
	if (Pos != std::wstring::npos) {
		UINT nLen = _tcslen(_T("您已获得"));
		Str_yezi = pStr.substr(Pos + nLen, pStr.find(_T("叶子"), Pos) - Pos - nLen);
	}
	else Str_yezi = L"None";
	Pos = pStr.find(_T("滑稽 头套"));
	if (Pos != std::wstring::npos) {
		UINT nLen = _tcslen(_T("滑稽 头套"));
		Str_huaji = pStr.substr(Pos + nLen, pStr.find(_T("天"), Pos) - Pos - nLen);
	}
	else Str_huaji = L"None";
	Pos = pStr.find(_T("BiliBili22娘"));
	if (Pos != std::wstring::npos) {
		UINT nLen = _tcslen(_T("BiliBili22娘"));
		Str_bili22 = pStr.substr(Pos + nLen, pStr.find(_T("天"), Pos) - Pos - nLen);
	}
	else Str_bili22 = L"None";
	CLabelUI* pLabel_bili22 = static_cast<CLabelUI*>(m_pm.FindControl(_T("Label_bili22")));
	if (pLabel_bili22) {
		std::wstring pText = _T("bilibili22娘 ");
		pText += Str_bili22;
		pText += _T(" 天");
