...
void C礼包UI::DoEvent(TEventUI& event)
{
...
void C礼包UI::OnClick(CControlUI* Click)
{
...
void C礼包UI::Notify(TNotifyUI& msg)
{
	if (_tcscmp(msg.sType, _T("click")) == 0)OnClick(msg.pSender);
}

_bstr_t C礼包UI::GetFormhash(LPCTSTR pUrl)
{
	std::string strHtml;
	lib_http::CLibhttp http;
...
void C礼包UI::online_Award(LPCTSTR pUrl, LPCTSTR szAward)
{
	//if (MessageBox(NULL, _T("由于X社接口限制原因,请求操作会很慢甚至假死. 请问是否继续操作?"), NULL, MB_YESNO) == IDNO)return;
	std::string Result;
	{
...
void C礼包UI::DrawCalendar(CContainerUI* pAwarad)
{
	SYSTEMTIME m_systime = { 0 };
	GetLocalTime(&m_systime);
	time_t TmpTime;
...
int C礼包UI::GetMonthDays(int iY, int iM)
{
	int iTotalDay = 31;
	if (iM == 2)
	{
