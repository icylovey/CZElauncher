...
public:
	...
	std::string GetChildValue(UINT nIndex)
	{
		std::string ResultValue;
		if (name == "head" || name == "meta" || name == "style" || name == "script" || name == "link")return ResultValue;
		if (name == "plain")ResultValue.append(value);
		if (children.size() >= nIndex)children[nIndex]->PlainStylize(ResultValue);
		return ResultValue;
	}
