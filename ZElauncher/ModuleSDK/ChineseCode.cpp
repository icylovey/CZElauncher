...
std::string strCoding::usc2ToAnsi(const std::string& instr)
{
	std::wstring str;
	char* pElementText;
	int iTextLen;
...
const unsigned long long CSHA512::sha512_k[80] = {
...
void CSHA512::update(const unsigned char* message, unsigned int len) {
	unsigned int block_nb; unsigned int new_len, rem_len, tmp_len; const unsigned char* shifted_message;
	tmp_len = SHA384_512_BLOCK_SIZE - m_len;
	rem_len = len < tmp_len ? len : tmp_len; memcpy(&m_block[m_len], message, rem_len); if (m_len + len < SHA384_512_BLOCK_SIZE) {
		m_len += len; return;
	}
	new_len = len - rem_len;
...
void CSHA512::final(unsigned char* digest) {
	unsigned int block_nb; unsigned int pm_len; unsigned int len_b; int i;
...
std::string Encode_sha512(std::string input) {
	unsigned char digest[CSHA512::DIGEST_SIZE]; memset(digest, 0, CSHA512::DIGEST_SIZE);
	CSHA512 ctx = CSHA512();
	ctx.init();
	...
	buf[2 * CSHA512::DIGEST_SIZE] = 0; for (int i = 0; i < CSHA512::DIGEST_SIZE; i++)sprintf(buf + i * 2, "%02x", digest[i]); return std::string(buf);
}
