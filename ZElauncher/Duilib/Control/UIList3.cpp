...
void CListContainerElementUI::DrawItemBk(HDC hDC, const RECT& rcItem)
{
	...
	if( !IsEnabled() ) {
		if( DrawImage(hDC, (LPCTSTR)pInfo->sDisabledImage, pInfo->diDisabled) ) return;
	}
	if( IsSelected() ) {
		if( DrawImage(hDC, (LPCTSTR)pInfo->sSelectedImage), pInfo->diSelected) ) return;
	}
	if( (m_uButtonState & UISTATE_HOT) != 0 ) {
		if( DrawImage(hDC, (LPCTSTR)pInfo->sHotImage, pInfo->diHot) ) return;
	}
	if( !DrawImage(hDC, m_sBkImage) ) {
		if( !pInfo->bAlternateBk || m_iDrawIndex % 2 == 0 ) {
			if( DrawImage(hDC, (LPCTSTR)m_sBkImage, pInfo->diBk) ) return;
		}
	}
}
