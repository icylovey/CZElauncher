...
LPCTSTR CMenuUI2::GetClass() const
{
    return kMenuUIClassName;
}

LPVOID CMenuUI2::GetInterface(LPCTSTR pstrName)
{
    if( _tcsicmp(pstrName, kMenuUIInterfaceName) == 0 ) return static_cast<CMenuUI2*>(this);
    return CListUI::GetInterface(pstrName);
}

void CMenuUI2::DoEvent(TEventUI& event)
{
	return __super::DoEvent(event);
}

bool CMenuUI2::Add(CControlUI* pControl)
{
	CMenuElementUI2* pMenuItem = static_cast<CMenuElementUI2*>(pControl->GetInterface(kMenuElementUIInterfaceName));
	if (pMenuItem == NULL)
		return false;
	...
	for (int i = 0; i < pMenuItem->GetCount(); ++i)
	{
		...
		{
			(static_cast<CMenuElementUI2*>(pMenuItem->GetItemAt(i)->GetInterface(kMenuElementUIInterfaceName)))->SetInternVisible(false);
		}
	}
	return CListUI::Add(pControl);
}

bool CMenuUI2::AddAt(CControlUI* pControl, int iIndex)
{
	CMenuElementUI2* pMenuItem = static_cast<CMenuElementUI2*>(pControl->GetInterface(kMenuElementUIInterfaceName));
	if (pMenuItem == NULL)
		return false;
...
int CMenuUI2::GetItemIndex(CControlUI* pControl) const
{
	CMenuElementUI2* pMenuItem = static_cast<CMenuElementUI2*>(pControl->GetInterface(kMenuElementUIInterfaceName));
	if (pMenuItem == NULL)
		return -1;
...
bool CMenuUI2::SetItemIndex(CControlUI* pControl, int iIndex)
{
	CMenuElementUI2* pMenuItem = static_cast<CMenuElementUI2*>(pControl->GetInterface(kMenuElementUIInterfaceName));
	if (pMenuItem == NULL)
		return false;
...
bool CMenuUI2::Remove(CControlUI* pControl, bool bDoNotDestroy)
{
	CMenuElementUI2* pMenuItem = static_cast<CMenuElementUI2*>(pControl->GetInterface(kMenuElementUIInterfaceName));
	if (pMenuItem == NULL)
		return false;
...
SIZE CMenuUI2::EstimateSize(SIZE szAvailable)
{
	int cxFixed = 0;
    int cyFixed = 0;
...
void CMenuUI2::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
...
class CMenuBuilderCallback: public IDialogBuilderCallback
{
	CControlUI* CreateControl(LPCTSTR pstrClass)
	{
		...
		{
			return new CMenuElementUI2();
		}
		return NULL;
	}
};

CMenuWnd2::CMenuWnd2(HWND hParent):
m_hParent(hParent),
m_pOwner(NULL),
m_pLayout(),
m_xml(_T(""))
...
BOOL CMenuWnd2::Receive(ContextMenuParam2 param)
{
	switch (param.wParam)
	{
	case 1:
...
void CMenuWnd2::Init(CMenuElementUI2* pOwner, STRINGorID xml, LPCTSTR pSkinType, POINT point, MenuEvent2 pMenuInfo)
{
	m_BasedPoint = point;
    m_pOwner = pOwner;
    m_pLayout = NULL;
	...
	m_xml = xml;
	if (pMenuInfo)g_menuevent = pMenuInfo;
...
LPCTSTR CMenuWnd2::GetWindowClassName() const
{
    return _T("MenuWnd");
}

void CMenuWnd2::OnFinalMessage(HWND hWnd)
{
	RemoveObserver();
	if( m_pOwner != NULL ) {
...
LRESULT CMenuWnd2::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if( uMsg == WM_CREATE ) {
		if( m_pOwner != NULL) {
			...
			m_pLayout = new CMenuUI2();
			m_pLayout->SetManager(&m_pm, NULL, true);
			LPCTSTR pDefaultAttributes = m_pOwner->GetManager()->GetDefaultAttributeList(kMenuUIInterfaceName);
			if( pDefaultAttributes ) {
				m_pLayout->SetAttribute(pDefaultAttributes,_T(""));
				//m_pLayout->SetAttribute(pDefaultAttributes)
			}
			...
			RECT rcPreWindow = {0};
			ContextMenuObserver2::Iterator2<BOOL, ContextMenuParam2> iterator(s_context_menu_observer);
			ReceiverImplBase2<BOOL, ContextMenuParam2>* pReceiver = iterator.next();
			while( pReceiver != NULL ) {
				CMenuWnd2* pContextMenu = dynamic_cast<CMenuWnd2*>(pReceiver);
	...
	else if( uMsg == WM_KILLFOCUS )
	{
		...
		ContextMenuObserver2::Iterator2<BOOL, ContextMenuParam2> iterator2(s_context_menu_observer);
		ReceiverImplBase2<BOOL, ContextMenuParam2>* pReceiver = iterator2.next();
		while( pReceiver != NULL ) {
			CMenuWnd2* pContextMenu = dynamic_cast<CMenuWnd2*>(pReceiver);
...
CMenuElementUI2::CMenuElementUI2():
m_pWindow(NULL)
{
	m_menuevet = nullptr;
	m_cxyFixed.cy = 25;
...
LPCTSTR CMenuElementUI2::GetClass() const
{
	return kMenuElementUIClassName;
}

LPVOID CMenuElementUI2::GetInterface(LPCTSTR pstrName)
{
    if( _tcsicmp(pstrName, kMenuElementUIInterfaceName) == 0 ) return static_cast<CMenuElementUI2*>(this);    
    return CListContainerElementUI::GetInterface(pstrName);
}

bool CMenuElementUI2::DoPaint(HDC hDC, const RECT& rcPaint, CControlUI* pStopControl)
{
    RECT rcTemp = { 0 };
    ...
    CMenuElementUI2::DrawItemBk(hDC, m_rcItem);
...
void CMenuElementUI2::DrawItemText(HDC hDC, const RECT& rcItem)
{
...
SIZE CMenuElementUI2::EstimateSize(SIZE szAvailable)
{
	SIZE cXY = {0};
	for( int it = 0; it < GetCount(); it++ ) {
		CControlUI* pControl = static_cast<CControlUI*>(GetItemAt(it));
...
void CMenuElementUI2::DoEvent(TEventUI& event)
{
	if( event.Type == UIEVENT_MOUSEENTER )
	{
		...
		for( int i = 0; i < GetCount(); ++i )
		{
			...
			{
				(static_cast<CMenuElementUI2*>(GetItemAt(i)->GetInterface(kMenuElementUIInterfaceName)))->SetVisible(true);
				(static_cast<CMenuElementUI2*>(GetItemAt(i)->GetInterface(kMenuElementUIInterfaceName)))->SetInternVisible(true);
	...
	if( event.Type == UIEVENT_BUTTONDOWN )
	{
		if( IsEnabled() ){
			...
			MenuInfo2 MInfo;
			MInfo.nIndex = GetIndex();
			MInfo.MenuText = GetText();
			if (g_menuevent)g_menuevent(g_hWnd, &MInfo);
        }
        return;
    }
...
CMenuWnd2* CMenuElementUI2::GetMenuWnd()
{
	return m_pWindow;
}

void CMenuElementUI2::CreateMenuWnd()
{
	if( m_pWindow ) return;

	m_pWindow = new CMenuWnd2(m_pManager->GetPaintWindow());
	ASSERT(m_pWindow);
	...
	m_pWindow->Init(static_cast<CMenuElementUI2*>(this), _T(""), _T(""), CDuiPoint(), nullptr);
}
