...
#pragma region 判断地图是否存在
	...
	if (!IsFileExist(mapName)) {
		if (MessageBox(NULL, _T("本地没有当前地图,是否下载地图后,进入游戏?"), _T("Tip"), MB_YESNO) == IDYES) {
			...
			if (sTempstr.find("X社区") != std::string::npos)CreateThread(NULL, NULL, (LPTHREAD_START_ROUTINE)CServerUI::DownloadMap93x, (LPVOID)g_mapName.GetData(), NULL, NULL);
			else if (sTempstr.find("僵尸乐园") != std::string::npos)CreateThread(NULL, NULL, (LPTHREAD_START_ROUTINE)CServerUI::DownloadMapjsly, (LPVOID)g_mapName.GetData(), NULL, NULL);
			else if (sTempstr.find("风云社") != std::string::npos)CreateThread(NULL, NULL, (LPTHREAD_START_ROUTINE)CServerUI::DownloadMapfys, (LPVOID)g_mapName.GetData(), NULL, NULL);
			else {
				OutPutTip2(_T("不支持的社区服,无法下载地图!"));
				RunGame(Serveraddr);
			}
...
void CServerUI::DownloadMap93x(LPCTSTR lpMapName)
{
	...
	if (http.DownloadFile(sUrl.GetData(), sWriteFile.GetData(), DownloadProc_)) {
		...
		if (Zip7ZUnCompressed(sWriteFile.GetData(), _T(""), sMapFile.GetData(), szOutPut, NULL)) {
			...
			else pServer->OutPutTip2(_T("解压失败,遇到未知错误!"));
		}
		pServer->RunGame(Serveraddr);
	}
	else {
		pServer->OutPutTip2(_T("地图下载失败,目前仅支持ZE僵尸逃跑地图!"));
		pServer->RunGame(Serveraddr);
	}
